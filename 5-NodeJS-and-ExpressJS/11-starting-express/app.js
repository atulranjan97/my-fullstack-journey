...
// Agar hum koi bhi path nahi de rahien hai, toh middleware har jo incoming request aa rahi hai uske liye applicable hai, aur chlega us sequence me jis sequence me humne middlewares ko lagaya hai.
...
// so agar koi sirf '/' ke liye aa raha hai to sirf dusre wala middleware chalega and ye middleware nahi chalega, but agar koi '/test' ke liye aa raha hai toh '/' path wala middleware and '/test' path wala middleware, ye dono humare middleware chalenge. 
...
/*
`app.use()` ko request ka method kya hai is baat se koi farak nahi padega, vo sirf path dekh raha hoga,

Agar aap sirf GET type ki request ko handle karna chah rahein hai kisi specific middleware se toh aap `app.get()` ka use kar sakte hai
Similarly, agar aap sirf POST type ki request ko handle karna chah rahein hai kisi specific middleware se toh aap `app.post()` ka use kar sakte hai
`app.get()` and `app.post()` path ki exact matching karte hai, jo `app.use()` nahi karta.
*/